...
def test_patterson_f3(fixture, api: AnophelesAdmixtureAnalysis):
    ...
    params = dict(
        ...
        segregating_mode=str(np.random.choice(["recipient", "all"])),
        n_jack=int(np.random.randint(10, 200)),
        min_cohort_size=1,
        max_cohort_size=50,
    )

    # Run patterson f3 function under test.
    f3, se, z = api.patterson_f3(**params)
    # Check results.
...
@parametrize_with_cases("fixture,api", cases=".")
def test_patterson_f4(fixture, api: AnophelesAdmixtureAnalysis):
    # Set up test parameters.
    all_taxon = api.sample_metadata()["taxon"].dropna().unique().tolist()
    taxa = np.random.choice(all_taxon, size=2, replace=False).tolist()
    ...
    # Run patterson f4 function under test.
    f4, se, z = api.patterson_f4(**params)
    # Check results.
    assert isinstance(f4, np.float64)
    assert isinstance(se, np.float64)
    assert isinstance(z, np.float64)
    assert -0.1 <= f4 <= 1
    assert 0 <= se <= 1
