...
class PlasmodiumData:
    def __init__(
        ...
        **kwargs,
        ...
        # Set resource
        self.resource_options = {"pf7": "pf7_config.json", "pv4": "pv4_config.json"}
        if resource in self.resource_options:
            self.resource = resource
        else:
            raise ValueError(
                "{} is not a data resource option. Options: {}".format(
                    resource, list(self.resource_options.keys())
                )
            )
    ...
    def _load_config(self, data_config):
        ...
        if not data_config:
            ...
            data_config = os.path.join(
                working_dir, self.resource_options[self.resource]
            )
